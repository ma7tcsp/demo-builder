...
function getImage(protocol,port,token, host,siteid, pat,viewid,wid) {
    return new Promise((resolve, reject)=>{
      ...
      fs.createReadStream(__dirname +'/public/empty.png').pipe(fs.createWriteStream(pat+'/'+viewid+'.png'));
...
function getFolder(pat){
  if (!fs.existsSync(__dirname +"/public/"+ pat)) {
    fs.mkdirSync(__dirname +"/public/"+pat);
  }
  return __dirname +"/public/"+pat;
}
...
function copyTemplate(tpname){
  ...
  let to=__dirname + '/'+Date.now().toString();
