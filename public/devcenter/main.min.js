...
function closeAllMenu(){$(".sidebar-dropdown").removeClass("active");$(".sidebar-submenu").hide()}function switchTemplate(a,b){$("#template").prop("src",a);currentTemplate=a;disposeAllViz();b&&closeSettings();$("#container").empty();clearInterval(isTemplateLoaded);restoreViz();b&&($(".thumb.templ").removeClass("active"),$(b).addClass("active"),$(".sidebar-menu .sidebar-submenu details .fas").hide(),$(b).find("i").show())}
...
function restoreViz(){setTimeout(function(){isTemplateLoaded=setInterval(function(){"undefined"!=typeof document.getElementById("template").contentWindow.loadVizInit&&(clearInterval(isTemplateLoaded),setTimeout(function(){restoreTexts();restoreImgs();restoreFilters();restoreViews();restoreFilters();restoreParameters();restoreWebEdit();restoreAskData();restoreAction();restoreColors()},500),$(".page-content").css("opacity","1"))},500)},1100)}
