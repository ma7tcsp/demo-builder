...
a.next=a.head=a},k=0;return d},"es6","es3");var VERSION="1.02",DEFAULT_SETTINGS="default",NO_FILTER_TEXT="Activate View at Least Once to Setup...",WARNTABS="This workbook has hidden tabs, switching views and keeping filter state could not be possible ! Consider changing to 'Tabbed Views'.",currentTemplate,crossMessageHandler,magicGrid,pickers=[],panelSet,viewsModified=!1,curSearch,curSearchVarC;Window.prototype._addEventListener=Window.prototype.addEventListener;
Window.prototype.addEventListener=function(a,b,c){"message"===a&&(crossMessageHandler=b);void 0==c&&(c=!1);this._addEventListener(a,b,c);this.eventListenerList||(this.eventListenerList={});this.eventListenerList[a]||(this.eventListenerList[a]=[]);this.eventListenerList[a].push({listener:b,options:c})};function addNewIframeListener(){for(var a=0;a<$("iframe").length;a++)$("iframe")[a].contentWindow.addEventListener("message",crossMessageHandler,!0)}
...
3);initModal();1==checkSettings()?($(".init").show(),$(".tabpub input").prop("checked",!1),getProjects()):(showSettings(),$(".tdropdown").show(),$(".tabpub").show(),$(".vdropdown").show(),$(".sidebar-search").show(),$(".tabpub input").prop("checked",!0));currentTemplate="templates/grid/index.html";restoreViz();initHelpVideo();a.jumpToEnd()})}
function initHelpVideo(){$("[data-media]").on("click",function(a){a.preventDefault();a=$(this).attr("data-media");$(".popup").find("iframe").attr("src",a);$("body").addClass("show-popup")});$(".popup").on("click",function(a){a.preventDefault();a.stopPropagation();$("body").removeClass("show-popup")});$(".popup > iframe").on("click",function(a){a.stopPropagation()})}
...
function saveSettings(){var a;return $jscomp.asyncExecutePromiseGeneratorProgram(function(b){if(1==b.nextAddress)return b.yield(giveMeSalt($("#tokValue").val()),2);a=b.yieldResult;localStorage.setItem("SYNC_FILTERS",$("#sync").prop("checked"));localStorage.setItem("SERVER_URL",$("#servurl").val());localStorage.setItem("SITE_NAME",$("#siteName").val());localStorage.setItem("TOKEN_NAME",$("#tokName").val());localStorage.setItem("TOKEN_VALUE",a);localStorage.setItem("VERSION",VERSION);$(".tabpub input").prop("checked",
!1);1!=checkSettings()||isTableauPublic()?togglePublic():(closeAllMenu(),clearItems(),$(".init").show(),$(".tdropdown").addClass("active"),$(".tdropsub").show(),getProjects());MicroModal.close("modal-settings");b.jumpToEnd()})}function getSyncSetting(){var a=localStorage.getItem("SYNC_FILTERS");"true"==a?a=!0:a=!1;return a}
function showSettings(){$("#template").contents().find("iframe").fadeOut(200);$("#sync").prop("checked",getSyncSetting());$("#servurl").val(localStorage.getItem("SERVER_URL"));$("#siteName").val(localStorage.getItem("SITE_NAME"));$("#tokName").val(localStorage.getItem("TOKEN_NAME"));$("#tokValue").val(localStorage.getItem("TOKEN_VALUE"));showModal("modal-settings");$(".gogo").focus()}
...
$("#"+a[0].id)),$(".wbdropdown").removeClass("active"),$(".wbropsub").hide(),$(".vdropdown").removeClass("active"),$(".vdropsub").hide(),d.jumpToEnd()}})}function closeAllMenu(){$(".sidebar-dropdown").removeClass("active");$(".sidebar-submenu").hide()}
function switchTemplate(a,b){b&&($(".thumb.templ").removeClass("active"),$(b).addClass("active"),$(".sidebar-menu .sidebar-submenu details .fas").hide(),$(b).find("i").show());$("#template").prop("src",a);currentTemplate=a;disposeAllViz();$("#container").empty();restoreViz()}function showViews(a,b,c){$(".thumb.wkb").removeClass("active");$(".vnum").text("");$(".loadview").show();$(b).addClass("active");$(".tb").scrollTop(0);getViews(a,c)}
function showWorkbook(a,b){$(".vnum").text("");$(".loadview").show();$(".thumb.proj").removeClass("active");$(".wbnum").text("");$(".loadwb").show();$(b).addClass("active");$(".twb").scrollTop(0);getWorkbooks(a)}function allowDrop(a){a.preventDefault()}function drag(a){a.dataTransfer.setData("text",a.target.id);$("#template").contents().find("iframe").fadeOut(200)}
function getDataSource(a){a.getUnderlyingTablesAsync().then(function(b){options={maxRows:0,ignoreAliases:!1,ignoreSelection:!0,includeAllColumns:!1};a.getUnderlyingTableDataAsync(b[0].getTableId(),options).then(function(a){console.log(a.getData(),a.getColumns())})})}function dropEnd(){$("#template").contents().find("iframe").fadeIn(200)}function disposeFilters(a){$("div").remove("div [vizf='"+a+"']")}
function disposeAllViz(){tableau.VizManager.getVizs().map(function(a){a.dispose();disposeFilters(a.vizid);console.log("Disposing",a.vizid,"for",a.template)})}
...
function initPicker(a,b){$("#"+a).css("color",contrastFontColor(b));var c=Pickr.create({el:"#"+a,theme:"nano",useAsButton:!0,default:b,i18n:{"btn:save":"OK"},swatches:"rgba(244, 67, 54, 1);rgba(0, 165, 81, 1);rgba(233, 30, 99, 0.95);rgba(156, 39, 176, 0.9);rgba(103, 58, 183, 0.85);rgba(63, 81, 181, 0.8);rgba(33, 150, 243, 0.75);rgba(3, 169, 244, 0.7);rgba(0, 188, 212, 0.7);rgba(0, 150, 136, 0.75);rgba(76, 175, 80, 0.8);rgba(76, 175, 80, 0.8);rgba(205, 220, 57, 0.9);rgba(255, 235, 59, 0.95);rgba(255, 193, 7, 1)".split(";"),components:{preview:!0,
opacity:!0,hue:!0,interaction:{input:!0,cancel:!0,save:!0}}}).on("save",function(b){$("#"+a).css("background-color",b.toRGBA().toString(0));$("#"+a).css("color",contrastFontColor(b.toHEXA().toString(0)));$("#"+a).attr("value",b.toHEXA().toString(0));c.hide()});$("#"+a).on("change",function(a){var b=$(a.currentTarget).val();$(a.currentTarget).css("background-color",b);$(a.currentTarget).css("color",contrastFontColor(b));c.setColor(b)});pickers.push({id:a,picker:c})}
...
b+="<li onchange=\"$('.filterText[dumid="+c.getName().replace(/[^a-z0-9+]+/gi,"_")+"]').toggle('hideFilterInput')\" class=\"param"+a+'" ><input class="param-entry" varindex="'+a+'" varc="'+c.getName()+'" type="checkbox" '+(1==c.isChecked?"checked":"")+"> "+c.getName()+" (Parameter)</li>"+d}});return b}
...
"-","")+'\')" class="fas target fa-bullseye"></i> <label class="label_settings">'+b+' </label>\n        <div class="picgroup">\n          <input varc="'+a.variable+'" class="imgs input input_settings searchpict" required="true" placeholder="Search for images or paste url below...">\n          <input varc="'+a.variable+'" class="imgs input input_settings sh" required="true" value="'+a.img+'">\n        </div>\n    </div>\n    ';$("#imglist").append(a)}),addSearchListener(),$("#colorlist").empty(),$("#colorlist").append("<summary>Colour Settings</summary>"),
parseForCSSVar(document.getElementById("template").contentWindow.document).map(function(a,c){var b='\n    <div class="settings_block">\n      <label class="label_settings">'+a.variable.replaceAll("-"," ")+' </label>\n      <input varc="'+a.variable+'" style="background-color:'+a.value+';" id="color'+c+'" class="color input input_settings" required="true" value="'+a.value+'">\n    </div>\n    ';$("#colorlist").append(b);initPicker("color"+c,a.value)}),$("#textlist").empty(),$("#textlist").append("<summary>Text Settings</summary>"),
parseForText(document.getElementById("template").contentWindow.document).map(function(a){var b="",d=(new RegExp(/Viz-([0-9-]+)-text/)).exec(a.variable);null!=d?($(".viewName[varindex='"+(parseInt(d[1])-1)+"']").prop("value",decodeURIComponent(a.text)),$(".viewName[varindex='"+(parseInt(d[1])-1)+"']").attr("varc",a.variable)):-1==a.variable.indexOf("Filter-text-")&&(b='\n    <div  class="settings_block">\n        <i onclick="highlightElement(\''+a.variable.replace(currentTemplate+"-","")+'\')" class="fas target fa-bullseye"></i><label class="label_settings">'+
a.variable.replace(currentTemplate+"-","").replaceAll("-"," ")+' </label>\n        <input varc="'+a.variable+'" class="texts input input_settings" required="true" value="'+decodeURIComponent(a.text)+'">\n    </div>\n    ');$("#textlist").append(b)}),collapseDetailsPanelAuto())}
...
b){saveToRepo("text","Filter-text-"+$(b).attr("varc"),encodeURIComponent($(b).prop("value")));restoreTexts()});$("#imglist .imgs").each(function(a,b){saveToRepo("img",$(b).attr("varc"),$(b).prop("value"));restoreImgs()});a&&closeSettings();1==viewsModified?(switchTemplate(currentTemplate),viewsModified=!1):$(".page-content").css("opacity","1")}
...
function restoreViews(){var a=getStorageByType("view");0<a.length&&(a=a[0].val.split(","),document.getElementById("template").contentWindow.tab_server=a,document.getElementById("template").contentWindow.loadVizInit())}function restoreImgs(){getStorageByType("img").map(function(a){$(document.getElementById("template").contentWindow.document.getElementById(a.key)).prop("src",a.val)})}
...
function restoreViz(){setTimeout(function(){var a=setInterval(function(){"undefined"!=typeof document.getElementById("template").contentWindow.loadVizInit&&(clearInterval(a),restoreColors(),restoreTexts(),restoreImgs(),restoreFilters(),restoreViews(),restoreFilters(),restoreParameters(),restoreWebEdit(),restoreAskData(),restoreAction(),$(".page-content").css("opacity","1"))},500)},500)}
...
$(d).find(".thumb_text").text().toUpperCase().indexOf(b)?($(d).show(),c+=1):$(d).hide()});$(".wkb").each(function(a,c){-1!=$(c).find(".thumb_text").text().toUpperCase().indexOf(b)?($(c).show(),e+=1):$(c).hide()});$(".vie").each(function(a,c){-1!=$(c).find(".thumb_text").text().toUpperCase().indexOf(b)?($(c).show(),d+=1):$(c).hide()});$(".pnum").text(c);$(".vnum").text(d);$(".wbnum").text(e)}}function closeSettings(){panelSet&&panelSet.close()}
function createSettingsPanel(){$("body").append(getSettingsTemplate());collapseDetailsPanelAuto();var a=document.getElementById("settings");panelSet=jsPanel.create({content:a,headerControls:"closeonly md",contentSize:{width:"740px",height:"500px"},contentOverflow:"hidden",headerTitle:"TEMPLATE SETTINGS",theme:"#31353D"})}
...
function exportTemplate(a){var b=getStorageByType("webedit"),c=getStorageByType("askdata"),d=getStorageByType("parameter"),e=getStorageByType("filter"),f=getStorageByType("view"),g=getStorageByType("img"),k=getStorageByType("text"),h=getStorageByType("color"),l=getStorageByType("action");b={title:a.replaceAll(".zip",""),tpname:currentTemplate.replace("templates/","").replace("/index.html",""),view:JSON.stringify(f),filter:JSON.stringify(e),parameter:JSON.stringify(d),webedit:JSON.stringify(b),askdata:JSON.stringify(c),
text:JSON.stringify(k),img:JSON.stringify(g),color:JSON.stringify(h),action:JSON.stringify(l)};b=formize(b);fetch("/zip",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:b}).then(function(a){return a.blob()}).then(function(b){saveFile(b,a.replaceAll(".zip","")+".zip")})}
...
function clearAllSettings(){MicroModal.close("modal-settings");showModal("modal-confirm")}function clearAllDamn(a){MicroModal.close("modal-confirm");1==a&&(localStorage.clear(),reloadMe(!0))}function openExportOptions(a){a.stopPropagation();showModal("modal-export");$(".gogo").focus();$("#exportname").val("grid-site.zip")}
function exportOptions(a){MicroModal.close("modal-export");1==a&&(exportTemplate($("#exportname").val()),setTimeout(function(){$("#exportconf").prop("checked")&&saveToFile($("#exportname").val())},1E3))}String.prototype.hashCode=function(){var a=0,b;if(0===this.length)return a;for(b=0;b<this.length;b++){var c=this.charCodeAt(b);a=(a<<5)-a+c;a|=0}return a};