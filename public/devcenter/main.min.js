...
function closeAllMenu(){$(".sidebar-dropdown").removeClass("active");$(".sidebar-submenu").hide()}function switchTemplate(a,b){closeSettings();b&&($(".thumb.templ").removeClass("active"),$(b).addClass("active"),$(".sidebar-menu .sidebar-submenu details .fas").hide(),$(b).find("i").show());$("#template").prop("src",a);currentTemplate=a;disposeAllViz();$("#container").empty();clearInterval(isTemplateLoaded);restoreViz()}
...
f);reloadMe(!0)}}function openExportOptions(a){a.stopPropagation();showModal("modal-export");$(".gogo").focus();$("#exportname").val("grid-site.zip")}function exportOptions(a){MicroModal.close("modal-export");1==a&&(exportTemplate($("#exportname").val()),setTimeout(function(){$("#exportconf").prop("checked")&&saveToFile($("#exportname").val())},1E3))}String.prototype.hashCode=function(){var a=0,b;if(0===this.length)return a;for(b=0;b<this.length;b++){var c=this.charCodeAt(b);a=(a<<5)-a+c;a|=0}return a};