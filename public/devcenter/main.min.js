...
function exportTemplate(a){var b=getStorageByType("webedit"),c=getStorageByType("askdata"),d=getStorageByType("parameter"),e=getStorageByType("filter"),f=getStorageByType("view"),g=getStorageByType("img"),k=getStorageByType("text"),h=getStorageByType("color"),l=getStorageByType("action");b={title:a.replaceAll(".zip",""),view:JSON.stringify(f),filter:JSON.stringify(e),parameter:JSON.stringify(d),webedit:JSON.stringify(b),askdata:JSON.stringify(c),text:JSON.stringify(k),img:JSON.stringify(g),color:JSON.stringify(h),
action:JSON.stringify(l)};b=formize(b);fetch("/zip",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:b}).then(function(a){return a.blob()}).then(function(b){saveFile(b,a.replaceAll(".zip","")+".zip")})}
