...
a.next=a.head=a},k=0;return d},"es6","es3");var VERSION="1.02",DEFAULT_SETTINGS="default",NO_FILTER_TEXT="Activate View at Least Once to Setup...",WARNTABS="This workbook has hidden tabs, switching views and keeping filter state could not be possible ! Consider changing to 'Tabbed Views'.",currentTemplate,crossMessageHandler,magicGrid,pickers=[],panelSet,viewsModified=!1,curSearch,curSearchVarC,isTemplateLoaded;Window.prototype._addEventListener=Window.prototype.addEventListener;
...
function closeAllMenu(){$(".sidebar-dropdown").removeClass("active");$(".sidebar-submenu").hide()}function switchTemplate(a,b){b&&($(".thumb.templ").removeClass("active"),$(b).addClass("active"),$(".sidebar-menu .sidebar-submenu details .fas").hide(),$(b).find("i").show());$("#template").prop("src",a);currentTemplate=a;disposeAllViz();$("#container").empty();clearInterval(isTemplateLoaded);restoreViz()}
...
function restoreViz(){setTimeout(function(){isTemplateLoaded=setInterval(function(){"undefined"!=typeof document.getElementById("template").contentWindow.loadVizInit&&(clearInterval(isTemplateLoaded),restoreColors(),restoreTexts(),restoreImgs(),restoreFilters(),restoreViews(),restoreFilters(),restoreParameters(),restoreWebEdit(),restoreAskData(),restoreAction(),$(".page-content").css("opacity","1"))},500)},500)}
