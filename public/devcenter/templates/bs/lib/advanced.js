...
function populateParameterMenuOld(param){
  var links="";
  if(param.getAllowableValues()==null){
    console.log("this param as no allowable values :"+param.getName());
    return;
...
function showActionIfExist(index){
  ...
  if(ids && ids.val && ids.val=="true")
    document.getElementsByClassName("action")[0].style.display = "inline-block";
}
function showWebEditIfExist(index){
  ...
  if(ids && ids.val && ids.val=="true")
    document.getElementsByClassName("webedit")[0].style.display = "inline-block";
}
function showAskButtonIfExist(index){
  ...
  if(ids && ids.val && ids.val!=""){
    document.getElementsByClassName("askdata")[0].style.display = "inline-block";
  }
}
...
function storeLastPageload(index){
  localStorage.setItem("lastpageloaded-bs",index);
}
function getLastPageload(index){
  var id=localStorage.getItem("lastpageloaded-bs");
  if(id!=null)
    return parseInt(id);
  return null;  
