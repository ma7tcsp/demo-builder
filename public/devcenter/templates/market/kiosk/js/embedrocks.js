...
if(tableau){
				...
				this.push = function (){
					...
					tableau.VizManager.getVizs().map((v)=>{
						if(!v.patched){
							...
							v.injectWhatEver= function Viz$InjectWhatEver(param) {
								sendCommand(this,"Inject",param);
							};
	...
	function sendCommand(_this,command,param=null){
		var returnHandler={
		...
		_this._impl[detectMsg].sendCommand(Object).call(_this._impl[detectMsg], param, returnHandler);
	}
}